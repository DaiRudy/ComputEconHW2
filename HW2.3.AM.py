# Updated MIT shock code with K0 set to steady state capital
import numpy as np
...
@jit(nopython=True)
def solve_value_function(assets, Pi, beta, r, w, income, max_iter=2000, tol=1e-6):
    a_size = len(assets)
    V = np.zeros((2, a_size))
...
# =====================
# 3. MIT Shock Simulation with Capital Endogeneity
# =====================
def simulate_MIT_shock(assets, Pi, beta, income, T_transition=21, N=5000):
    ...
    z_shock = 2.74
    _, V_shock, policy_shock, _, _ = capital_equilibrium(z_shock)

    print("Simulating transition path...")
    # Start from simulated steady-state distribution
    wealth, employment = simulate_wealth_distribution(policy_steady, assets, Pi, T=2000, N=N)
    K_path = [K_steady]  # initialize with equilibrium K
    ...
    for t in range(1, T_transition):
        ...
        for i in range(N):
            a_cur = wealth[i]
            i_a = np.argmin(np.abs(assets - a_cur))
            a_prime = current_policy[employment[i], i_a]
            new_wealth[i] = manual_clip(a_prime, a_min, a_max)
            new_employment[i] = 0 if np.random.random() < Pi[employment[i], 0] else 1
        wealth = new_wealth
        employment = new_employment
...
if __name__ == "__main__":
    ...
    plt.title("Capital Transition Path after MIT Shock")
    plt.grid(True)
    plt.legend()
    plt.show()